...
open class ClusterManager {
    ...
    lazy var tree = QuadTree(rect: mapView.contentBounds, projection: projection)
    ...
    open var visibleNestedMarkers: [NMFMarker] {
        return dispatchQueue.sync {
            visibleMarkers.reduce([NMFMarker](), {
                $0 + (($1 as? ClusterMapMarker)?.markers ?? [$1])
            })
        }
    }
    ...
    open func removeAll() {
        ...
        dispatchQueue.async(flags: .barrier) { [weak self] in
            ...
            self.tree = QuadTree(rect: self.mapView.contentBounds,
    ...
    open func clusteredMarkers(zoomScale: Double, visibleMapRect: NMGLatLngBounds, operation: Operation? = nil) -> (toAdd: [NMFMarker], toRemove: [NMFMarker]) {
        ...
        if !shouldRemoveInvisibleMarkers {
            let toKeep = toRemove.filter { !visibleMapRect.hasPoint($0.position) }
    ...
    func clusteredMarkers(tree: QuadTree, mapRects: [NMGLatLngBounds], zoomLevel: Double) -> [NMFMarker] {
        ...
        for mapRect in mapRects {
            ...
            if count >= minCountForClustering, zoomLevel <= maxZoomLevel {
                let rect = projection.viewBounds(from: mapRect)
                let cluster = ClusterMapMarker()
                cluster.position = coordinate(markers: markers, position: clusterPosition, mapRect: rect)
                cluster.markers = markers
                cluster.style = (markers.first as? MapMarker)?.style
                allMarkers += [cluster]
            } else {
                allMarkers += markers
            }
        ...
        dispatchQueue.async(flags: .barrier) {
            for value in hash.values where value.count > 1 {
                for (index, marker) in value.enumerated() {
                    ...
                    let oldCoordinate = CLLocationCoordinate2D(latitude: marker.position.lat,
                                                               longitude: marker.position.lng)
                    let newCoordinate = oldCoordinate.coordinate(
                        onBearingInRadians: bearing,
                        atDistanceInMeters: self.distanceFromContestedLocation
                    )
                    marker.position = NMGLatLng(from: newCoordinate)
                    tree.add(marker)
                }
            }
        }
